...
def train(model, start):
    ...
    # 定义 Adam 优化器
    # model.parameters() 保存了 Weights 和 Bais 参数值
    ...
    while iteration < model.number_of_iterations:
        ...
        if iteration % 25000 == 0:
            torch.save(model, "dqn_pretrained_train_network/current_model_" + str(iteration) + ".pth")

        print("iteration:", iteration, "elapsed time:", time.time() - start, "epsilon:", epsilon, "action:",
              action_index.cpu().detach().numpy(), "reward:", reward.numpy()[0][0], "Q max:",
              np.max(output.cpu().detach().numpy()))
    
    with open('dqn_reward.csv', 'w') as csvfile:
...
def main(mode):
    ...
    if mode == 'test':
        model = torch.load(
            'dqn_pretrained_train_network/current_model_2000000.pth',
        ...
        if not os.path.exists('dqn_pretrained_train_network/'):
            os.mkdir('dqn_pretrained_train_network/')
